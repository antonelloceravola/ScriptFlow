...
const FLAGS = {
  ...
  LogTextInitial: true,
  LogTextFull: false,
  GroupConsecutiveTextIntoParagraphs: true,
  GroupFullChapter: true,
...
function buildChapterFilePath(chapterIndex, chapterTitle) {
  const safeTitle = sanitizeFileName(chapterTitle || `chapter-${chapterIndex}`)
                    .slice(0, 60); // Limit length to prevent filesystem issues  
  const fileName = `${String(chapterIndex).padStart(2, "0")}-${safeTitle}.aiff`;
...
async function flushChapterBuffer(chapterBuffer, chapterTitle, chapterIndex) {
  ...
  try {
    if (FLAGS.GenerateChapterAudioFiles) {
      ...
      const outputFile = buildChapterFilePath(chapterIndex, chapterTitle);
...
function convertFlagValue(rawValue, currentValue) {
...
async function flushChapterChunkBuffer() {
  if (!FLAGS.GroupFullChapter || !FLAGS.GenerateChapterAudioFiles) {
    return;
  }
...
async function main() {
  ...
  for (const item of items) {
    ...
    if (item.type === "chapter" && item.lineNumber < FLAGS.StartAfterLine) {
      currentChapterTitle = item.text;
    }

    if (!started) {
      if (item.lineNumber >= FLAGS.StartAfterLine) {
        console.log(`--- Starting from line ${FLAGS.StartAfterLine} ---`);
        started = true;
    ...
    if (item.type === "param") {
      ...
      if (FLAGS.GroupFullChapter && FLAGS.GenerateChapterAudioFiles && speechSettingChanged) {
        await flushChapterChunkBuffer();
      }
