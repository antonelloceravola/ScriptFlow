...
const NarratorPresets = {
  En1: {
    "Narrator":   { voice: "Lee" }, //"Zoe" },
    "Boy":        { voice: "Nathan" },
    "Girl":       { voice: "Joelle" }, //"Ava" },
    "Serious":    { voice: "Kate" },
  },
  It1: {
    "Narrator":   { voice: "Emma" },
...
function updateFLAGS(optionName, newValue) {
  ...
  if( optionName === "Language" ) {
    ...
    switch (newValue) {
      case "En1":
        FLAGS.Language = "En1";
        FLAGS.SayVoice = NarratorPresets.En1.Narrator.voice; //"Lee/Zoe";
        FLAGS.SayRate = RatesPresets.Neutral.rate; //165;
        result = true;
        break;
      case "It1":
        FLAGS.Language = "It1";
        FLAGS.SayVoice = NarratorPresets.It1.Narrator.voice; //"Emma";
        FLAGS.SayRate = RatesPresets.Neutral.rate; //165;
        result = true;
        break;
    }
    ...
    pushChapterContent(`[[slnc ${newValue}]]`);
  }
  return result;
}
...
async function consoleLogger(type, message, item) {
  ...
  if (FLAGS.SpeakText ) {
    ...
    switch (type) {
      ...
      case "section":
      case "subsection":
        prefix = ( headingPrefixes[FLAGS.Language]?.[type] || type )+
                 `[[slnc ${FLAGS.ShortPause}]]`; break;
    }
    try {
      if( prefix && item.text.toLowerCase().trim().startsWith(type) ) {
        prefix = "";
      }
